...
for gen in generic_name: 
    gen_val = Generic(name=gen,is_active = active, created_at = fake.date_time(), updated_at =fake.date_time())
    gen_val.save()

for sub_gen in sub_generic_name: 
    sub_gen_val = SubGeneric(name=sub_gen,is_active = active, created_at = fake.date_time(), updated_at =fake.date_time())
    sub_gen_val.save()

for unit in unit_name:
    unit_val = Unit(name=unit, created_at = fake.date_time(), updated_at =fake.date_time())
    unit_val.save()

for itemtype in item_type:
    item_val = ItemType(name=itemtype, created_at = fake.date_time(), updated_at =fake.date_time())
    item_val.save()
...
for i in range(len(discount)):
    discount_val = Discounts(name=discount[i],percentage = discount_percentage[i],is_fixed_amount =fixed_amount[i],created_at = fake.date_time(), updated_at =fake.date_time())
    discount_val.save()
